...
function App() {
  ...
  const contractAddress = "0x2Fb54e692C42499a2a4bC0B8191fd9C8B96a9DC2";
  ...
  const getTokenInfo = async () => {
    try {
      if (window.ethereum) {
        // A provider lets us connect to the ethereum blockchain, in this case the Rinkeby testnet via an ethereum node.
        // We're using MetaMask's provider which uses Infura behind the scenes.
        const provider = new ethers.providers.Web3Provider(window.ethereum);
        // Now with our provider we get a signer which is an abstraction of your MetaMask wallet that lets you interact with the blockchain without revealing your private keys.
        // As a signer, you can write to the ethereum blockchain via transactions
        ...
        // we're using our setter functions to set our initial state variables to these new values so they can be rendered into our app.
  ...
  const transferToken = async (event) => {
    ...
    try {
      if (window.ethereum) {
        ...
        console.log("Transfering Taavi bucks...");
        await txn.wait();
        console.log("Bucks transferred", txn.hash);
      } else {
